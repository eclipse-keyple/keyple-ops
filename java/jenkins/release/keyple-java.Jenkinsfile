...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SNAPSHOT_VERSION', defaultValue: '1.1.0-SNAPSHOT', description: 'Next development version.')
    }
    environment {
    ...
    stages {
        ...
        stage('Keyple Java: Build and Test next Release') {
            steps{
                container('java-builder') {
                    sh "./gradlew setVersion -Pversion=${RELEASE_VERSION}"
        ...
        stage('Keyple Android: Build and Test next Release') {
            steps{
                container('java-builder') {
                    dir('android') {
                        sh "./gradlew setVersion -Pversion=${RELEASE_VERSION}"
        ...
        stage('Keyple Examples: Build and Test next Release') {
            steps{
                container('java-builder') {
        ...
        stage('Keyple Java: Prepare next development') {
            steps{
                container('java-builder') {
                    sh "./gradlew setVersion -Pversion=${SNAPSHOT_VERSION}"
                    dir('android') {
                        sh "./gradlew setVersion -Pversion=${SNAPSHOT_VERSION}"
                    }
                    withCredentials([usernamePassword(credentialsId: 'github-bot', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        sh """
                            ...
                            git push 'https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/eclipse/keyple-java.git' develop
                        """
                    }
                }
            }
